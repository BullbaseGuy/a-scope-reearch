...
from ascope.status import reconcile_live
...
def command_fixture(args: argparse.Namespace) -> int:
    ...
    build_dashboard(
        shortlist,
        args.output_dir / "A_SCOPE_fixture_dashboard.html",
        "A-SCOPE Fixture Regression",
    )
...
def command_reconcile_status(args: argparse.Namespace) -> int:
    manifest = reconcile_live(
        args.security_master,
        args.output_dir,
        args.as_of_date,
        minimum_listed_count=args.minimum_listed_count,
        minimum_bse_count=args.minimum_bse_count,
    )
    print(json.dumps(manifest, ensure_ascii=False, indent=2))
...
def main() -> int:
    ...
    p = sub.add_parser("reconcile-status")
    p.add_argument("--security-master", type=Path, required=True)
    p.add_argument("--output-dir", type=Path, required=True)
    p.add_argument("--as-of-date", required=True)
    p.add_argument("--minimum-listed-count", type=int, default=5000)
    p.add_argument("--minimum-bse-count", type=int, default=150)
    p.set_defaults(func=command_reconcile_status)
    ...
    p.set_defaults(
        ...
        if validate_bundle(
            a.input_dir,
            a.as_of_date,
            minimum_securities=a.minimum_securities,
            minimum_market_days=a.minimum_market_days,
        )["status"]
        == "PASS"
        else 2
    )
    ...
    p.set_defaults(
        func=lambda a: (
            print(
                json.dumps(
                    run(a.input_dir, a.output_dir, a.as_of_date, mode=a.mode),
                    ensure_ascii=False,
                    indent=2,
                )
            ),
    ...
    p.set_defaults(
        func=lambda a: (
            lambda r: (
                ...
                0 if r["status"] == "PASS" else 2,
            )[1]
        )(validate_output(a.output_dir, a.mode))
    )
