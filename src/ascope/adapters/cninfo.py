...
def normalize(
    item: dict[str, Any], source_url: str, as_of_date: str, retrieved_at: str
) -> dict[str, Any] | None:
    ...
    return {
        ...
        "board": board,
        "status": "REVIEW",
        "status_reason": "CNINFO_IDENTITY_MAP_IS_NOT_CURRENT_STATUS",
        "is_st": bool("ST" in name.upper()),
        "list_date": item.get("ssrq") or item.get("listDate") or None,
        "as_of_date": as_of_date,
        "source_id": "CNINFO_BULK_IDENTITY",
...
def validate_universe(frame: pd.DataFrame, minimum_count: int) -> dict[str, Any]:
    ...
    if len(frame) < minimum_count:
        errors.append(f"incomplete identity map: {len(frame)} < {minimum_count}")
    ...
    return {
        ...
        "errors": errors,
        "status_semantics": "IDENTITY_ONLY_REQUIRES_RECONCILIATION",
    }
...
def discover(
    ...
    manifest = {
        "status": "IDENTITY_DISCOVERY_COMPLETE_REQUIRES_STATUS_RECONCILIATION"
        if validation["status"] == "PASS"
        else "DISCOVERY_INCOMPLETE",
        "provider": "CNINFO_BULK_IDENTITY",
        "expected_requests": 0 if payload_paths else len(URLS),
        "source_status": source_status,
        "validation": validation,
        "as_of_date": as_of_date,
        "generated_at_utc": retrieved,
        "investment_use": "PROHIBITED_UNTIL_STATUS_MARKET_AND_FINANCIAL_DATA_VALIDATED",
    }
